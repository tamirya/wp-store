...
if ($max_value && $min_value === $max_value) {
    ...
    ?>
    <div class="quantity order-md-1">
        <label class="screen-reader-text"
               for="<?php echo esc_attr($input_id); ?>"><?php esc_html_e('Quantity', 'woocommerce'); ?></label>
        <input type="button" value="-" class="qty_button minus btn btn-outline-secondary"/>
        <input
                type="number"
                id="<?php echo esc_attr($input_id); ?>"
                class="input-text qty text btn btn-outline-secondary"
        ...
        <input type="button" value="+" class="qty_button plus btn btn-outline-secondary"/>
    </div>
    <?php
}
